...
class QuadTree():
    ...
    def draw_line_regions(self, quadrant, draw, show_lines=False):
        ...
        if show_lines:
            draw.polygon([(int(x),int(y)) for x,y in above_line[above_hull.vertices]], int(quadrant.p1_avg * 255), outline=0)
            draw.polygon([(int(x),int(y)) for x,y in below_line[below_hull.vertices]], int(quadrant.p2_avg * 255), outline=0)
        else:
    ...
    def create_image(self, custom_depth, show_lines=False):
        ...
        for quadrant in leaf_quadrants:
            if show_lines:
                draw.rectangle(quadrant.bbox, int(quadrant.avg * 255), outline=0)
                if quadrant.isline:
